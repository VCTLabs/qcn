...
public:
    ...
    void db_parse(MYSQL_ROW& r)
    {
      ...
      if (std::isnan(significance)) significance = 0;
      magnitude = safe_atof(r[i++]);
      if (std::isnan(magnitude)) magnitude = 0;  // some reason sig &  or mag can be NaN
      mxy1p = safe_atof(r[i++]);
      mz1p = safe_atof(r[i++]);
      mxy1a = safe_atof(r[i++]);
      mz1a = safe_atof(r[i++]);
      ...
      mxy4a = safe_atof(r[i++]);
      mz4a = safe_atof(r[i++]);
      if (std::isnan(mxy1p)) mxy1p = 0;
      if (std::isnan(mxy1a)) mxy1a = 0;
      if (std::isnan(mxy2a)) mxy2a = 0;
      if (std::isnan(mxy4a)) mxy4a = 0;
      if (std::isnan(mz1p)) mz1p = 0;
      if (std::isnan(mz1a)) mz1a = 0;
      if (std::isnan(mz2a)) mz2a = 0;
      if (std::isnan(mz4a)) mz4a = 0;
      latitude = safe_atof(r[i++]);
      longitude = safe_atof(r[i++]);
      levelvalue = safe_atof(r[i++]);
      levelid = safe_atoi(r[i++]);
